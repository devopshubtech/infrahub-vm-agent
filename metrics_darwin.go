...
// metrics_darwin.go reads real host metrics via gopsutil -- this agent
// runs as a plain native macOS process, never inside a Docker Desktop
// container (Docker has no native macOS container concept at all -- it's
// always a Linux VM under the hood, which would report that VM's own
// resource view instead of the physical Mac's).
package main
...
// instead of reimplementing delta tracking -- gopsutil caches the
// previous sample internally for exactly this use.
...
// readLoadAvg: macOS has a real getloadavg()-backed load average, unlike
// Windows -- gopsutil's Darwin backend calls it natively.
...
// readRootStorage reports the real disk Macos itself is installed on,
// not any container/VM view.
